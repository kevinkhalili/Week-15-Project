...
@Component
@Slf4j
public class DefaultJeepSaleDao implements JeepSalesDao{
  ...
  public List<Jeep> fetchJeeps(JeepModel model, String trim) {
    log.debug("Dao: model={} and trim={}", model, trim);
    
    // @formatter: off
    String sql = ""
        + "SELECT + "
        + "FROM models "
        + "WHERE model_id = :model_id AND tirm_level = :trim_level";
    // @formatter: on
    
    Map<String, Object> params = new HashMap<>();
    params.put("model_id", model.toString());
    params.put("trim_id", trim);
